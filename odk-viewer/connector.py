...
class odk_connector:
    ...
    def get_forms(self):
        ...
        def get_submissions_from_form(self, form_id, form_namespace):
            ...
            while not list_complete:
                ...
                slist_xml = xmle.etree.ElementTree.fromstring(sub_list.content)
                ...
                # Get the new cursor.
                ncursor = xml.etree.ElementTree.tostring(cursor.getchildren()[0])
            ...
            # TODO fill this with information about the form (titles, etc)
            result['data'] = {}
            ...
            for uuid in uuids:
                ...
                formId = '{fid}[@version=null and @uiVersion=null]/{ns}[@key={val}]'.format(
                          fid=form_id, ns=form_namespace, val=uuid )
                
                subdata = requests.get(down_url, auth=self.auth,
                                       paramas={'formId':formId})
...
if __name__ == '__main__':
    ...
    conn = odk_connector(sys.argv[1], sys.argv[2], sys.argv[3])
    print conn.get_forms()
